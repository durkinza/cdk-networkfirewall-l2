...
describe("Testing Stateful Rule Group Features", () => {
  let stack: cdk.Stack;
  beforeEach(() => {
    // GIVEN
    stack = new cdk.Stack();
  ...
  test("Default properties on Domain List Group", () => {
    // WHEN
    new NetFW.StatefulDomainListRuleGroup(
      stack,
      ...
      "MyStateful5TupleRuleGroup",
      {
        ...
        referenceSets: {
          ipSetReferences: {
            MyIpSet: {
              referenceArn:
                "arn:aws:ec2:us-east-1:123456789012:prefix-list/pl-12345",
            },
          },
        },
      },
  ...
  test("Can get stateful rule group by name", () => {
    // GIVEN
    const statefulDomainListRuleGroup =
      NetFW.StatefulDomainListRuleGroup.fromRuleGroupArn(
        stack,
        "MyImportedDomainListRuleGroup",
        "arn:aws:networkfirewall:statefulrulegroup",
      );
    const statefulSuricataRuleGroup =
      NetFW.StatefulSuricataRuleGroup.fromRuleGroupArn(
        stack,
        "MyImportedSuricataRuleGroup",
        "arn:aws:networkfirewall:statefulrulegroup",
      );
    const stateful5TupleRuleGroup =
      NetFW.Stateful5TupleRuleGroup.fromRuleGroupArn(
        stack,
        "MyImportedStateful5TupleRuleGroup",
        "arn:aws:networkfirewall:statefulrulegroup",
      );
    ...
    new cdk.CfnResource(stack, "Res1", {
      type: "Test::Resource",
      properties: {
        statefulDomainListRuleGroupArn:
          statefulDomainListRuleGroup.ruleGroupArn,
      },
    });
  ...
  test("Can get stateless rule group from file", () => {
    // WHEN
    NetFW.StatefulSuricataRuleGroup.fromFile(
      stack,
      "MyStatefulSuricataRuleGroup",
      {
