...
test("Can get firewall by name", () => {
  // GIVEN
  const stack = new cdk.Stack();
  const firewall = NetFW.Firewall.fromFirewallName(
    stack,
    "MyNetworkFirewall3",
    "MyFirewall",
  );
...
test("firewall name is verified", () => {
  ...
  expect(() => {
    ...
    "firewallName must be non-empty and contain only letters, numbers, and dashes, got: 'MyFirewall%3'",
  );
});
...
test("Can provide Policy by Arn", () => {
  ...
  const policy = NetFW.FirewallPolicy.fromFirewallPolicyArn(
    stack,
...
test("Can provide full props", () => {
  ...
  Template.fromStack(stack).hasResourceProperties(
    "AWS::NetworkFirewall::Firewall",
    {
      FirewallName: "MyFirewall",
      FirewallPolicyArn: {
        "Fn::GetAtt": ["MyNetworkFirewallPolicy645720A6", "FirewallPolicyArn"],
      },
      SubnetMappings: [
        {
          SubnetId: {
