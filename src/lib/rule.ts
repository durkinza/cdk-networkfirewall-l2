...
export class StatelessRule extends StatelessRuleBase {
  ...
  constructor(props: StatelessRuleProps) {
    ...
    for (action of props.actions) {
      if (Object.values<string>(StatelessStandardAction).includes(action)) {
        if (standard_action_provided) {
          throw new Error(
            "Only one standard action can be provided, all other actions must be custom",
          );
        }
    ...
    const destinations: CfnRuleGroup.AddressProperty[] = castAddressProperty(
      props.destinations,
    );

    const sources: CfnRuleGroup.AddressProperty[] = castAddressProperty(
      props.sources,
    );
...
export class Stateful5TupleRule extends StatefulRuleBase {
  ...
  constructor(props: Stateful5TupleRuleProps) {
    ...
    const resourceOptions: CfnRuleGroup.RuleOptionProperty[] =
      props.ruleOptions || [];
